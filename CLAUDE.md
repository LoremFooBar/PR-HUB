...
**PR Tower bridge:** PR Tower (`~/repos/pr-tower-web`) is a separate local tool listing the same PRs. It is a served page, so it has no `chrome.tabs`/`chrome.tabGroups` of its own — a custom URL scheme would not help either, since handler URLs must be HTTP(S) and land on a page with the same limitation. Instead the content script marks a loopback page with `data-pr-hub="1"` (`src/bridge.ts`), PR Tower intercepts a plain left-click on a PR link and posts `{type: "prhub:open-pr", url}`, and the content script relays it to the service worker, which calls `openPRInGroup` (`src/tabs.ts`) — focus the tab if that PR is already open, otherwise create it, either way inside the "My PRs" group. Three checks keep it from being an open door: the content script accepts messages only from its own window at a **loopback** origin, and the service worker re-checks `sender.origin` and passes the URL through `normalizePRUrl`, which returns null for anything that is not a github.com pull-request URL. The marker goes on *any* loopback page, so another local dev server could also ask for a PR tab; the URL check bounds what that can do to opening a GitHub PR.
...
- `SearchBar` — Client-side filter for the active tab. Its query is applied via the pure `filterPRs` (`src/utils/search.ts`, token-AND over title/repo/branch/author, plus the PR number: a `#215` token matches that number only, a bare `215` matches the number or the text) memoized in `Dashboard`. Ephemeral (not persisted); `/` focuses, `Esc` clears.
